...
<?php
 ...
 $sql = "select count(dp.idserie), d.dispositivo from detalleproductos dp join productos p on dp.idproducto = p.idproducto join tipoproductos tp on p.idtipoproducto = tp.idtipoproducto join dispositivos d on tp.iddispositivo = d.iddispositivo where dp.idsituacion = '1' group by d.iddispositivo ";
 ...
 $Test = new pChart(450,180);
 $Test->setFontProperties("../sic/pChart/Fonts/tahoma.ttf",9);
 $Test->drawFilledRoundedRectangle(7,7,450,220,5,240,240,240);
 $Test->drawRoundedRectangle(5,5,450,220,5,230,230,230);
 ...
 $Test->clearShadow();
 $Test->drawPieLegend(250,30,$DataSet->GetData(),$DataSet->GetDataDescription(),250,250,250);
 $Test->drawTitle(50,22,"Productos por Perifericos en Almacen",50,50,50,585);
 $Test->Render("pChart/grafico_perifericos_productos.png");
